...
export async function getComparisons() {
  ...
  do {
    const data = await fetchAPI(
      `/comparisons?populate=*&sort=published_date:desc&pagination[page]=${page}&pagination[pageSize]=100`
    );
    allComparisons = allComparisons.concat(data.data || []);
    pageCount = data.meta?.pagination?.pageCount || 1;
    page++;
  } while (page <= pageCount);
...
export async function getComparisonBySlug(slug) {
  const data = await fetchAPI(
    `/comparisons?filters[slug][$eq]=${encodeURIComponent(slug)}&populate=*`
  );
  return (data.data && data.data[0]) || null;
}
...
// -----------------------------------------------------------------------------
// Cross-category related articles, based on shared glossary term matches
// rather than just same-category filtering. See
// cross-category-related-articles-implementation-plan.md for the full design.
// -----------------------------------------------------------------------------

let _termProfilesCache = null;

// Computes, once per build, every article's matched glossary terms. Cached
// at module scope so this expensive step (fetch all articles + glossary,
// then run matchGlossaryTerms on each) runs exactly once per build, not
// once per article page.
export async function getArticleTermProfiles() {
  if (_termProfilesCache) return _termProfilesCache;
  ...
  _termProfilesCache = articles.map((article) => ({
    slug: article.slug,
    title: article.title,
    category: article.category,
    termSlugs: matchGlossaryTerms(article.content, glossaryTerms).map((t) => t.slug),
  }));

  return _termProfilesCache;
}

export async function getRelatedArticlesByTermOverlap(currentSlug, limit = 4) {
  const profiles = await getArticleTermProfiles();
  const current = profiles.find((p) => p.slug === currentSlug);
  if (!current || current.termSlugs.length === 0) return [];

  const scored = profiles
    .filter((p) => p.slug !== currentSlug)
    .map((p) => ({
      ...p,
      overlap: p.termSlugs.filter((t) => current.termSlugs.includes(t)).length,
    }))
...
// -----------------------------------------------------------------------------
// Hub Pages — topical landing pages aggregating articles, products, tools,
// and case-study content under one URL. See
// cluster1-hub-pages-strategy.md for the full design.
// -----------------------------------------------------------------------------

export async function getHubPages() {
  const data = await fetchAPI('/hub-pages?populate=*');
  return data.data || [];
}

export async function getHubBySlug(slug) {
  const data = await fetchAPI(
    `/hub-pages?filters[slug][$eq]=${encodeURIComponent(slug)}` +
      `&populate[related_products][populate]=cover_image` +
      `&populate[related_articles][populate][0]=cover_image` +
      `&populate[related_articles][populate][1]=category` +
      `&populate[related_tool_links]=true`
  );
  return (data.data && data.data[0]) || null;
}

// -----------------------------------------------------------------------------
// Ask the Engineer — Q&A feature. See ask-the-engineer-implementation-plan.md
// and ask-the-engineer-strapi-setup.md for the full design and Strapi setup.
// -----------------------------------------------------------------------------
// Public-facing questions only ever come from review_status = "answered".
// The "pending" and "declined" values are internal workflow states and
// should never be queryable by the public API filter below (double-check
// this is also enforced by your Strapi permissions, not just this filter,
// per the setup doc's security note).
...
export async function getQuestions() {
  ...
  do {
    const data = await fetchAPI(
      `/questions?filters[review_status][$eq]=answered&populate=*&sort=answered_date:desc&pagination[page]=${page}&pagination[pageSize]=100`
    );
    allQuestions = allQuestions.concat(data.data || []);
    pageCount = data.meta?.pagination?.pageCount || 1;
    page++;
  } while (page <= pageCount);
...
export async function getQuestionBySlug(slug) {
  const data = await fetchAPI(
    `/questions?filters[slug][$eq]=${encodeURIComponent(
      slug
    )}&filters[review_status][$eq]=answered&populate=*`
  );
  return (data.data && data.data[0]) || null;
}

export async function getQuestionsByCategorySlug(slug, limit = 4) {
  const data = await fetchAPI(
    `/questions?filters[category][slug][$eq]=${encodeURIComponent(
      slug
    )}&filters[review_status][$eq]=answered&populate=*&sort=answered_date:desc&pagination[pageSize]=${limit}`
  );
  return data.data || [];
}
