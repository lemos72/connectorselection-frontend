...
export async function getArticles() {
  const data = await fetchAPI(
    '/articles?populate=*&sort=published_date:desc&pagination[pageSize]=500'
  );
  return data.data || [];
}
...
export async function getBlogPosts() {
  const data = await fetchAPI(
    '/blog-posts?populate=*&sort=published_date:desc&pagination[pageSize]=500'
  );
  return data.data || [];
}
...
export async function getProductIntroductions() {
  const data = await fetchAPI(
    '/product-introductions?populate=*&pagination[pageSize]=500'
  );
  return data.data || [];
}
